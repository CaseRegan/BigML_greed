...
# is part of my implementation of greed

POSSIBLE_MOVES = [[x, y] for x in [-1, 0, 1] for y in [-1, 0, 1]]


def gen_board(size):
    board = [[random.randint(1, min(9, size - 1))
             for x in range(size)]
             for y in range(size)]
...
def serialize(tree):
    inputs = [[element for row in t[1] for element in row] + t[2]
              for t in tree][1:]
...
def get_space_value(board, cursor):
    ...
    if cursor[0] >= 0 and cursor[1] >= 0 \
            and cursor[0] < size and cursor[1] < size:
...
def move(board, cursor, move):
    ...
    tmp_board = [[get_space_value(board, [x, y]) for y in range(size)]
                 for x in range(size)]
...
def score(board):
    return [element for row in board for element in row].count(0) \
        / len(board) ** 2


def minimax(board, cursor, prev_move=None, depth=0, tree=[]):
    children = [[move(board, cursor, p_move), POSSIBLE_MOVES.index(p_move)]
                for p_move in POSSIBLE_MOVES]
    legal_children = [child for child in children if child[0]]
    ...
    for child in legal_children:
        child_board, child_cursor, child_move, child_tree \
            = minimax(child[0][0], child[0][1], child[1], depth + 1, tree)
...
state = {'field%d' % (i + 1): random.randint(1, BOARD_SIZE - 1)
         for i in range(16)}
...
for i in range(NUM_ITER):
    ...
    t = minimax_wrap(b, c)
    s = serialize(t)[0]  # only using first board to reduce complexity
    f.write(s + '\n')
